  ...
  def reserve
    respond_to do |format|
      if @book.update!(status: 1, user: current_user)
        format.js { render nothing: true, notice: 'Book (libro) fue reservado.' }
      end
    end
  end
  ...
  def anti_reserve
    respond_to do |format|
      if @book.update!(status: 0, user: nil)
        format.js { render nothing: true, notice: 'Book (libro) fue desreservado.' }
      end
    end
  end
  ...
  def buy
    respond_to do |format|
      if @book.update!(status: 2, user: current_user)
        format.js { render nothing: true, notice: 'Book (libro) fue comprado.' }
      end
    end
  end
